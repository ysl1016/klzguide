...
function HeroCompareContent({ locale }: { locale: string }) {
  ...
  const factionNames: Record<string, { ko: string; vi: string }> = {
    wingsOfDawn: { ko: '윙즈 오브 던', vi: 'Wings of Dawn' },
    bloodRose: { ko: '블러드로즈', vi: 'Blood Rose' },
    guardOfOrder: { ko: '가드 오브 오더', vi: 'Guard of Order' },
  };
        ...
        <Card className="border-highlight/30 bg-highlight/5">
          <CardContent className="p-4">
            ...
            <p className="text-sm text-muted-foreground">
              {isKorean
                ? '메인 진형: 같은 진영 5명 (히든 버프 발동). 추천: 윙즈 오브 던 5명 (서버 70%가 블러드로즈 선택으로 카운터 유리) 또는 윙즈 3 + 블러드 2 조합.'
                : 'Đội hình chính: 5 người cùng phe (kích hoạt buff ẩn). Khuyến nghị: 5 Wings of Dawn (70% server chọn Blood Rose nên counter có lợi) hoặc 3 Wings + 2 Blood.'}
            </p>
          </CardContent>
        </Card>
      </div>
